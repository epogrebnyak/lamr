...
def markdown_items(lp: LearningPoint):
    return _markdown_items(
        lp,
        header_level=2,
        questions_header="Questions and excercises",
        continue_headers=True,
    )
...
src = from_toml("variables.toml")["topics"]
for title, sections in src.items():
    content = str(Header(title, 1))
    for section in sections:
        content += "\n\n"
        content += to_markdown(LearningPoint(**section))
    filename = title.lower().replace(" ", "_") + ".md"
    (pathlib.Path("docs") / filename).write_text(content, encoding="utf-8")
